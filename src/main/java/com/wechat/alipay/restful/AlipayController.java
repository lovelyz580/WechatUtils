package com.wechat.alipay.restful;
import java.util.Date;
import java.util.HashMap;
import java.util.Iterator;
import java.util.Map;
import javax.servlet.http.HttpServletRequest;
import com.wechat.alipay.Config.AlipayConfig;
import com.wechat.alipay.entity.OrderStatusEnum;
import com.wechat.alipay.entity.Orders;
import com.wechat.alipay.util.LeeJSONResult;
...
public class AlipayController {
    ...
    public ModelAndView goConfirm(String productId) throws Exception {
        ...
        System.out.println("选择商品   商品详情返回页面");
        return mv;
    }
...
//        获取产品的详情信息
        String orderId = ("11112222");  //生成一个随机id
        order.setId(orderId);
        order.setOrderNum(orderId);
        order.setCreateTime(new Date());
        order.setOrderAmount(String.valueOf(200));
        order.setOrderStatus(OrderStatusEnum.WAIT_PAY.key);
//        orderService.saveOrder(order);   //将order 存入数据库
...
//        mv.addObject("p", p);
    ...
    public LeeJSONResult createOrder(Orders order) throws Exception {
         ...
         * 保存订单
         */
        String orderId = "2233eeeerr"; // 生成随机id
        order.setId(orderId);
        order.setOrderNum(orderId);
        order.setCreateTime(new Date());
        order.setOrderAmount(String.valueOf(300));
        order.setOrderStatus(OrderStatusEnum.WAIT_PAY.key);
//        orderService.saveOrder(order); //  將order 保存至数据库
...
//        Orders order = orderService.getOrderById(orderId);   //查询订单
//        Product p = productService.getProductById(order.getProductId());   //查询产品
        ModelAndView mv = new ModelAndView("goPay");
//           返回确认支付页面
        return mv;
    }
...
//        Orders order = orderService.getOrderById(orderId); //查询订单
//        Product product = productService.getProductById(order.getProductId()); //查询产品
        //获得初始化的AlipayClient
...
//            orderService.updateOrderStatus(out_trade_no, trade_no, total_amount); //更新订单信息
